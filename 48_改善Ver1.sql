SELECT PHJD.PHJD_KHA_COD AS KHA_COD,
       MSKMD.SKMD_SZK_COD AS SZK_COD,
       PPJD1.PPJD_KM_COD AS KM_COD,
       PHJD.PHJD_GHM_LV1_COD AS GHM_LV1_COD,
       PHJD.PHJD_KM_COD AS IRS_KM_COD,
...
INNER JOIN BGTA3_PPJD PPJD ON PPJD.PPJD_KHA_COD = PHJD.PHJD_KHA_COD
   AND PPJD.PPJD_KM_COD = PHJD.PHJD_KM_COD
   AND PPJD.PPJD_PJ_COD = PHJD.PHJD_PJ_COD
INNER JOIN BGTAZ_PTCD PTCD ON PTCD.PTCD_KHA_COD = PHJD.PHJD_KHA_COD
   AND PTCD.PTCD_PJ_COD = PHJD.PHJD_PJ_COD
INNER JOIN BGTA2_SKMD MSKMD ON MSKMD.SKMD_KHA_COD = PHJD.PHJD_KHA_COD
   AND MSKMD.SKMD_KBS_VER_ID = '60'
   AND MSKMD.SKMD_KM_COD = PHJD.PHJD_KM_COD
...
WHERE PHJD.PHJD_KHA_COD = '20512'
   ...
   AND PHJD.PHJD_YSS_DTK_COD = '3'
   AND PHJD.PHJD_YSS_DTB_COD = '82'
   AND EXISTS (SELECT 1 FROM BGTA3_PPJD PPJD1
               WHERE PPJD1.PPJD_KHA_COD = PPJD.PPJD_KHA_COD
                 AND PPJD1.PPJD_SZK_COD = PPJD.PPJD_IRM_SZK_COD
                 AND PPJD1.PPJD_PJ_COD = PPJD.PPJD_IRM_PJ_COD
                 AND PPJD1.PPJD_SNI_TRH_KBN = '13')
GROUP BY PHJD.PHJD_KHA_COD,
         MSKMD.SKMD_SZK_COD,
         PPJD1.PPJD_KM_COD,
         PHJD.PHJD_GHM_LV1_COD,
         PHJD.PHJD_KM_COD,
         SKMD.SKMD_SZK_COD,
         PHJD.PHJD_TAS_YDO
