/* 頻繁に実行され、計算コストが高いクエリに対しては、
マテリアライズドビューを使用するか、クエリ結果を保存するキャッシュテーブルを作成します。
データの変更が頻繁でない場合、この方法は非常に効果的です。
マテリアライズドビューまたはキャッシュテーブルを定期的に更新することで、
リアルタイム計算の必要性を大幅に減少させることができます。
*/
-- 创建物化视图
...
INNER JOIN
    BGTA2_SKMD SSKMD ON SSKMD.SKMD_KHA_COD = PPJD.PPJD_KHA_COD
