...
----------------------------------------------------
    ...
    Set ws = ThisWorkbook.Sheets("Sheet1")  ' 确保工作表名称正确
    Dim lastRow As Long
    lastRow = ws.Cells(ws.Rows.Count, "D").End(xlUp).Row  ' 假设数据在D列
...
Sub ProcessBatch(ws As Worksheet, startRow As Long, endRow As Long)
    ...
    For i = startRow To endRow
        Set dict = CreateObject("Scripting.Dictionary")
        For j = startRow To endRow
            If ws.Cells(j, 4).Value = ws.Cells(i, 4).Value Then  ' 假设A数据在D列
                dict(ws.Cells(j, 5).Value) = 1  ' 假设B数据在E列
            End If
        Next j
        
        ' 判断是否对应多个不同的值
        If dict.Count > 1 Then
            ws.Cells(i, 26).Value = "多条"  ' 结果输出到Z列
        Else
            ws.Cells(i, 26).Value = "单条"
        End If
    Next i
End Sub
