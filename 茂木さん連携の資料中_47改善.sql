...
おそらく私はビジネスデータについての理解が不十分かもしれませんが、サブクエリを使用したSQLは確かに速いです。JOINを使用したSQLの実行計画には「buffer sort」が含まれています。
JOINを使用したSQLの実行計画には「buffer sort」が含まれていますが、サブクエリを使用した場合には「buffer sort」が含まれていません。


私の第一の提案は、クエリ全体をビューとして作成することです。そうすることで、速度が向上すると考えています。


データベースの環境を変更できないため、データ量が多く、JOINの条件に含まれるいくつかの列がインデックス対象でない場合、サブクエリを使用したほうが速度が速いとも感じています。



まだ理解できない部分が多いので、ご指摘をよろしくお願いします。
頻繁にJOINされる列については、インデックスを設定するのが良いと思いますが、使用しているデータベースでは簡単に変更できないかもしれません。
